          ...
          <div style={{ width: '100%', maxWidth: 420 }}>
            ...
            <div style={{ marginBottom: 32, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: T4 }}>
                {mode === 'login' ? 'Access your Strategy Execution Suite' : mode === 'signup' ? 'Get started with SMR' : 'Enter your email to receive a reset link'}
              </div>
            </div>
            ...
            <div style={{ marginTop: 20, textAlign: 'center', display: 'flex', justifyContent: 'center', gap: 20 }}>
              {mode !== 'login' && (
                <button onClick={() => { setMode('login'); setError(''); setMessage('') }} style={{ background: 'none', border: 'none', ...mono, fontSize: 10, color: T4, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', textDecoration: 'underline' }}>
                  Sign In
                </button>
              )}
              {mode !== 'signup' && (
                <button onClick={() => { setMode('signup'); setError(''); setMessage('') }} style={{ background: 'none', border: 'none', ...mono, fontSize: 10, color: T4, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', textDecoration: 'underline' }}>
                  Create Account
                </button>
              )}
              {mode !== 'reset' && (
                <button onClick={() => { setMode('reset'); setError(''); setMessage('') }} style={{ background: 'none', border: 'none', ...mono, fontSize: 10, color: T4, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', textDecoration: 'underline' }}>
                  Forgot Password
                </button>
              )}
            </div>
